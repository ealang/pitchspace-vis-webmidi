...
(function() {
  ...
  decodeMidiEvent = function(event) {
    var d1, d2, d3, noteOff, noteOn, ref;
    ref = event.data, d1 = ref[0], d2 = ref[1], d3 = ref[2];
    noteOn = (d1 & 0xF0) === 0x90;
  ...
  MidiInstance = (function() {
    function MidiInstance(access) {
      ...
      this.onmidimessage = (function(_this) {
    ...
    MidiInstance.prototype.selectDevice = function(id) {
      if (this.selectedInput !== null) {
        this.inputs[this.selectedInput].onmidimessage = void 0;
      }
      this.selectedInput = id;
      return this.inputs[id].onmidimessage = this.onmidimessage;
    };
    ...
    layers = (function() {
      ...
      return {
        ...
        settingsTonics: (function() {
          var j, len, results;
          results = [];
          for (j = 0, len = tonicLists.length; j < len; j++) {
            l = tonicLists[j];
            results.push(d3.select(l));
          }
          return results;
        })(),
  ...
  App = (function() {
    function App(view, midi) {
      ...
      midi.onKeyPress(function(event) {
        var ref, ref1;
        if (event.noteOn === true && (ref = event.noteNum, indexOf.call(activeNotes, ref) >= 0) === false) {
          activeNotes.push(event.noteNum);
          view.drawNoteAttack(event.noteNum, event.velocity);
        } else if (event.noteOn === false && (ref1 = event.noteNum, indexOf.call(activeNotes, ref1) >= 0)) {
  ...
  colorGen = function(note_num) {
    ...
    return "#" + ((function() {
      var j, len, results;
      results = [];
      for (j = 0, len = comp.length; j < len; j++) {
        c = comp[j];
        digit = Math.floor((c + m) * 255).toString(16);
        if (digit.length === 2) {
          results.push(digit);
        } else {
          results.push("0" + digit);
        }
      }
      return results;
    })()).join("");
